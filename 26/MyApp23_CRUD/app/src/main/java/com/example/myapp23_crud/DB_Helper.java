...
//------------------------------------------------ delete record
    public  boolean Delete_Record(int del_id)
    {
        SQLiteDatabase db = this.getWritableDatabase();
      return   db.delete(TABLE_NAME,"Std_ID = "+del_id,null) >0;

    }
    //------------------------------------------------ Update record
    public  void Update_Record(String up_id,String up_name,String up_email,String up_bc)
    {
        SQLiteDatabase db = getWritableDatabase();
        ContentValues cv = new ContentValues();
        cv.put(Col_1,up_id);
        cv.put(Col_2,up_name);
        cv.put(Col_3,up_email);
        cv.put(Col_4,up_bc);
        String whereclause="Std_ID=?";
        String whereargs[] = {up_id};
